...
#Look for nice example of rescue
```{r}
onetwo<-na.omit(inner_join(int_1, int_2, by="query_region", relationship = "many-to-many"))
onetwo_sign<-subset(onetwo, p_val.y.x < 0.01 | p_val.y.y<0.01)
onetwo_sub<-onetwo_sign[,c(8,15,20,49)]

##Transform dataframe to GRanges acceptable format
row_names <- onetwo_sub$query_region
split_names <- strsplit(row_names, "-")
onetwo_sub$seqnames <- sapply(split_names, `[`, 1)
onetwo_sub$start <- sapply(split_names, `[`, 2)
onetwo_sub$end <- sapply(split_names, `[`, 3)
onetwo_sub<-unique(onetwo_sub)
onetwo_sub$ID <- seq.int(nrow(onetwo_sub))
row.names(onetwo_sub)<-onetwo_sub$ID 
granges <- GenomicRanges::makeGRangesFromDataFrame(onetwo_sub[,c(5:7)])
...
p3 <- CoveragePlot(
  ...
  region.highlight = granges[11:12]
)
...
ggsave("/scratch/csierra/multiome_osk/plots/4_integration_Prnp.pdf", height = 5, width = 7, dpi=600, plot=p4)
```
